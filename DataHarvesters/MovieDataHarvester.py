...
def harvest_data_from_themoviedb():
    ...
    for x in range(5):
        ...
        for movie in json_movies['results']:
            ...
            Movie.objects.create(movie['title'], movie["id"], timedelta(hours=2, minutes=30), movie['vote_average'],
                                 movie['release_date'], datetime.strptime(movie['release_date'], '%Y-%m-%d'), actors)
